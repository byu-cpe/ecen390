...
Verify that your receiver can output a net 100 mV RMS FFT signal received
from an LED in a laser tag unit excited with a 0 - 3.3V, 1 - 5kHz square
wave at a distance of 40 feet (_net_ meaning the difference from the
largest noise signal).

You will be using the laser tag unit mounted on the wall in the lab for
the light source. The tag unit repeatedly transmits 3 frequencies (1250
Hz, 2000 Hz, 3636 Hz) in rapid sussession for 56 ms each. Use an
oscilloscope at a test station in the rear of the Analog Lab to capture
one-half second of data from the output of your receiver. Save the raw
data from the oscilloscope on a USB flash drive in CSV format. The raw
data needs to be collected with a [sample
rate](https://youtu.be/I2x_nUwriFQ) of 100 ksamples/second.

{: .note }
You can use the [FFT capability](https://youtu.be/bAspQY1Sg_A?t=33) of the
oscilloscope to check if you meet the specification before capturing the
CSV file that you will submit. Since 3 frequencies are being sent in the
sampling window, the RMS FFT signal for each frequency should be 33 mV or
above (100 mV / 3).

Oscilloscope settings:

  - Horizontal scale: 50 ms/
  - Under the save menu select: Format CSV
  - Under the save menu select: Settings, Length 50000
...
View the [Laser Tag Kit Assembly Video](https://youtu.be/Nahy_cQTVWU) for helpful instructions. Kit assembly occurs in five main steps:
...
Your kit does not contain the LiPo battery for safety reasons. When you are ready to insert your battery, you can obtain one from the instructor or a TA. It is included in the price of your kit.

Your main digital board comes unprogrammed. If you power on your unit at this stage, only the red power indicator near the power switch should illuminate. In some cases, depending on the design of your analog board, your power LED may illuminate. The next step is necessary to verify the functionality of your assembled kit.
...
assignment through Gradescope.

{: .important }
The CSV file should be time-domain data sampled by the oscilloscope
and not a screen shot.

Requirements:
 - Sample rate 100 kHz
 - Sample length 0.5 seconds
 - Each of the 3 signals minus noise must be 33 mV or above
 - The maximum signal minus the minimum signal divided by the
   average of the three signals must be less than 0.707
