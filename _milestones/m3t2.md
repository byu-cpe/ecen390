...
<!-- For further help in configuring the pin, see this
[GPIO example](https://github.com/espressif/esp-idf/tree/v5.2.3/examples/peripherals/gpio/generic_gpio). -->

The trigger component uses callback functions to signal when the
trigger has been pressed or released. The function to be called must be
"registered" with the trigger component and saved for later use. This is
done by passing a function pointer as an argument to the
`trigger_register_...()` functions. You will need to create global
variables to save the argument values. If a callback function has not
been registered, then no function should be called for that event. See
this reference on
...
Mechanical switches are imperfect when operated. When pushed or released
...
a reasonable assumption for a switch that is in good working order.
  ...
  - Step 2: Once you detect the press of the switch, start a counter
    that expires at 40 ms.
  - Step 3: If the switch changes value before the counter expires,
...
#### Specification
  ...
  - Each time the transmitter driver is activated with a call to
    ...
    duration in ms.
  - Use an [ESP32 software timer](https://docs.espressif.com/projects/esp-idf/en/stable/esp32s3/api-reference/system/freertos_idf.html#timer-api)
...
channel. This [LEDC example](https://github.com/espressif/esp-idf/tree/v5.2.3/examples/peripherals/ledc/ledc_basic)
...
ledc_timer_config_t members
  ...
  - clock configuration: LEDC_USE_XTAL_CLK
...
Also, `tx_init()` must create an instance of an ESP32 software timer using
`xTimerCreate()`. Configure the timer to be a one-shot timer. See this
[reference](https://docs.espressif.com/projects/esp-idf/en/stable/esp32s3/api-reference/system/freertos_idf.html#timer-api)
for an example of how to initialize and use the timer.
...
an instance of an ESP32 software timer using `xTimerCreate()`. Configure
...
## Test and Pass Off
  ...
  <!-- - Connect an oscilloscope probe to the TX output pin (J2 pin 4)
    to display the output of the transmitter for debug and pass off. -->
