...
Task 1. An implementation of the receiver driver (rx) is provided for you.
The receiver driver interacts with the ADC, which digitizes the amplified
signal coming from the photodiode on your analog board. You will test each
of these components with provided test code to verify correct operation.
In a later milestone, you will be guided how to connect these components
in a complete laser-tag system.
...
code. The test code calls functions in buffer.c and in detector.c. The
test results are printed to the terminal window.
