...
Each group (typically 2 students) will:
  ...
  - Verify functionality using a pair of laser tag units
    ...
    - Flash the 'hit indicator' (NeoPixels) when hit
...
and trigger. The display shows several control and status items.
  ...
  - Control panel:
    - Transmit mode (TX): shot - 200 ms pulse when trigger is pressed,
...
other to receive. The transmitting unit will need to be located 40 feet
away from the receiver.

1.  Power-on or reset both units. The default channel should be 6.
1.  Set the transmitting unit to channel 8.
1.  Focus the light beam of the transmitting unit while temporarily
    setting the TX mode to on. Point the beam at a wall or other
    light colored surface at least 10 feet away to see the shot pattern.
1.  Highlight the threshold factor control (THRESH) using the NAV Left
    and Right buttons.
...
{: .important }
Set the threshold factor of each tag unit to one increment lower than
the maximum values determined in the sensitivity test.
...
7.  Demonstrate that there are no false positives
    1.  Point Unit 2 at the room lights and demonstrate that hits are
        not recorded
    2.  Point Unit 1 so that a shot will miss Unit 2
    3.  Pull the trigger and make sure that a hit is not recorded
