...
The battery on your tag unit is charged when a USB cable is connected **and the tag unit power switch is turned on**. A mini, yellow-green LED will indicate that the battery is being charged. It will turn off when the battery is fully charged.
...
1.  When not being hit, all RGB LEDs must show the team color
    which is determined by the selected transmit channel (0-9).
2.  The team color must match the color of the channel on the
    histogram display (e.g., blue for Ch 6 and green for Ch 8).
    For red team colors, the brightness can be lower so that a
    hit (bright red) can be discerned.
3.  Flash all RGB LEDs red when a hit occurs.
...
The display shows several control and status items.
  ...
  - Control panel:
    ...
    - Volume (VOL): sound volume 0-100 in increments of 10
