...
filter and an array of instances for the energy computations. For the IIR
filter, use a 2D array of filter data type (filter_data_t) to hold the
state. The size of the first dimension is the number of filters, and the
size of the second is the number of state variables per section times the
...
the delay instances. The 2D array of IIR state can be set to zero with
...
The difference equation for a second-order section of an IIR filter
is shown below.
...
signal histories, but rather a 2D array as described in
[Filter Initialization](#filter-initialization).
In this case, we will be shifting the values in the "state" memory.

<!-- TODO: Need more detail about setting up pointers to each
sections memory -->
